...
def best_partition(graph, partition = None, second_stage = True) :
    ...
    >>> plt.show()
    """
    dendo = generate_dendogram(graph, partition, second_stage = second_stage)
    return partition_at_level(dendo, len(dendo) - 1 )


def generate_dendogram(graph, part_init = None, second_stage = True) :
    ...
    #implement nondeterministic Louvain (MM)
    ...
    while True :
        ...
        if second_stage == True:
            print "Performing second Louvain stage."
            current_graph = induced_graph(partition, current_graph)
            status.init(current_graph)
        else:
            print "Skipping second Louvain stage."
    return status_list[:]
