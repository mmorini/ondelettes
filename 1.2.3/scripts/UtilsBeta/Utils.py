...
def remove_accents(input_str):
    nkfd_form = unicodedata.normalize('NFKD', input_str.decode("utf-8"))
...
def defColumns(line,database):
  ...
  #
  if database == 'wos':
    Cols = ['PT', 'AU', 'TI', 'SO', 'DT', 'DE', 'ID', 'AB', 'C1', 'RP', 'CR', 'TC', 'J9', 'PY', 'LA', 'VL', 'IS', 'BP', 'WC', 'DI', 'UT']
    foo = line.replace('\xef\xbb\xbf','').split('\t')
  elif database == 'scopus':
    Cols = ['Authors', 'Title', 'Source title', 'Document Type', 'Author Keywords', 'Index Keywords', 'Abstract', 'Affiliations', 'References', 'Cited by', 'Abbreviated Source Title', 'Year', 'Volume', 'Issue', 'Page start', 'Language of Original Document', 'DOI', 'EID']
    foo = line.replace('\xef\xbb\xbf','').split(',')
        ...
        try:
            ...
            for line in fd.readlines():
                ...
                if (line != ""):
                    ...
                    if(len(s)>12): aline.uniqueID = s[12]
